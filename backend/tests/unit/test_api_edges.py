...
def _user() -> SimpleNamespace:
    ...
    return SimpleNamespace(
        id=uuid.uuid4(), username="alice", email="alice@example.com", full_name="Alice",
        avatar_url=None, is_active=True, is_superuser=True, is_protected=False,
        last_login_at=None, created_at=now, groups=[], global_permissions=[],
    )
...
async def test_attachment_endpoint_errors_and_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    attachment = SimpleNamespace(page_id=uuid.uuid4(), content_type="text/plain", filename='a\\"\n.txt', object_key="k")
    page = SimpleNamespace(space_id=uuid.uuid4())
...
async def test_simple_read_endpoints_and_backup_validation() -> None:
    ...
    search_service = Mock(search=AsyncMock(return_value=SimpleNamespace(query="wiki", pages=[], spaces=[])))
    ...
    settings_service = Mock(read_sidebar_permissions=AsyncMock(return_value=SimpleNamespace(permissions={})), read=AsyncMock(return_value="read"), update=AsyncMock(return_value="updated"))
    ...
    assert await site_settings.update_settings(SimpleNamespace(), settings_service, _user()) == "updated"

    document = BackupDocument(
        wikihub_backup=BackupMeta(version=1, exported_at=datetime.now(UTC), app_version="x", site_name="WikiHub", includes_credentials=False),
    )
    backup_service = Mock(export_document=AsyncMock(return_value=document), import_document=AsyncMock(return_value=ImportReport(dry_run=True, version=1, includes_credentials=False)))
    ...
    good_file = UploadFile(filename="backup.json", file=io.BytesIO(document.model_dump_json().encode()))
    report = await backup.import_backup(backup_service, good_file, True)
    assert report.dry_run is True
    with pytest.raises(BadRequestError):
        await backup.import_backup(backup_service, UploadFile(filename="x", file=io.BytesIO(b"nope")), True)
    with pytest.raises(BadRequestError):
        await backup.import_backup(backup_service, UploadFile(filename="x", file=io.BytesIO(b"{}")), True)
...
async def test_user_administration_endpoint_wrappers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    service = Mock(
        ...
        create_user=AsyncMock(return_value=target), update_user=AsyncMock(return_value=target),
        reset_password=AsyncMock(return_value=target), delete_user=AsyncMock(),
        update_own_profile=AsyncMock(return_value=target), change_password=AsyncMock(return_value=target),
    )
    ...
    assert await users.create_user(UserCreate(username="newuser", email="new@example.com", password="password1"), actor, session, service)
    assert await users.update_user(target.id, UserUpdate(full_name="New"), actor, session, service)
    assert await users.reset_user_password(target.id, PasswordReset(new_password="password1"), actor, session, service)
    assert await users.update_own_profile(SelfProfileUpdate(full_name="Alice"), actor, service)
    assert await users.change_own_password(PasswordChange(current_password="old", new_password="password1"), actor, service)
...
async def test_storage_and_dependency_edges(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    storage = Mock(
        ...
        exists=AsyncMock(return_value=True), delete=AsyncMock(),
    )
    ...
    assert (await storage_admin.presign_download(user, storage, key="a/b.txt", inline=False)).url == "https://download"

    session = Mock(scalar=AsyncMock(side_effect=[None, None]), flush=AsyncMock(), delete=AsyncMock())
    ...
    assert await deps.get_impersonator(SimpleNamespace(headers={}, cookies={}), Mock(get_active_user=AsyncMock())) is None
    ...
    assert await deps.get_impersonator(request, Mock(get_active_user=AsyncMock(return_value=user))) is user
    plain_token, _ = create_access_token(str(user.id))
    assert await deps.get_impersonator(SimpleNamespace(headers={"Authorization": f"Bearer {plain_token}"}, cookies={}), Mock()) is None
    with pytest.raises(AuthenticationError):
        await deps.get_impersonator(request, Mock(get_active_user=AsyncMock(side_effect=AuthenticationError("gone"))))
...
async def test_group_endpoint_wrappers(monkeypatch: pytest.MonkeyPatch) -> None:
    actor = _user()
    group = SimpleNamespace(id=uuid.uuid4(), name="Engineering", description="Docs", owner_id=actor.id, is_active=True, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
    owner = SimpleNamespace(username="alice")
    session = Mock(
        get=AsyncMock(return_value=owner), scalar=AsyncMock(return_value=2),
        scalars=AsyncMock(return_value=[GlobalPermission.manage_groups]),
        execute=AsyncMock(), add=Mock(),
    )
    ...
    assert await groups.create_group(GroupCreate(name="Engineering", owner_id=actor.id), actor, service, session)
    ...
    session.execute.return_value = Mock(scalars=Mock(return_value=Mock(all=Mock(return_value=[member]))))
...
async def test_confluence_import_endpoint_guards(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    archive = SimpleNamespace(
        id=uuid.uuid4(), filename="a.zip", size_bytes=10, sha256=None, status="scanned",
        error=None, spaces=[{"key": "ENG", "name": "Engineering", "page_count": 1, "attachment_count": 0}],
        object_key="imports/a.zip",
    )
    now = datetime.now(UTC)
    job = SimpleNamespace(
        id=uuid.uuid4(), archive_id=archive.id, import_all=False, space_keys=["ENG"],
        overwrite_existing=False, status="queued", phase="queued", counters={},
        cancel_requested=False, error=None, created_at=now, updated_at=now,
    )
    assert confluence_import.archive_read(archive).filename == "a.zip"
    assert confluence_import.job_read(job).id == job.id
    importer = Mock(session=Mock(), upload_part_size_bytes=8, start_upload=AsyncMock(return_value=archive), get_archive=AsyncMock(return_value=archive), uploaded_part_numbers=AsyncMock(return_value=[1]), upload_part_urls=AsyncMock(return_value={1: "url"}), complete_upload=AsyncMock(return_value=archive), abort_upload=AsyncMock(), create_job=AsyncMock(return_value=job))
    monkeypatch.setattr(confluence_import, "get_storage", Mock(return_value=Mock()))
    assert confluence_import.service(Mock())
    monkeypatch.setattr(confluence_import, "SiteSettingsService", lambda _session: SimpleNamespace(get_effective=AsyncMock(return_value=SimpleNamespace(max_backup_import_size_bytes=100))))
    target = await confluence_import.start_upload(UploadInit(filename="a.zip", size_bytes=10), actor, importer)
    assert target.archive_id == archive.id
    assert await confluence_import.get_upload_progress(archive.id, actor, importer)
    assert await confluence_import.get_upload_part_urls(archive.id, UploadPartUrlsRequest(part_numbers=[1]), actor, importer)
    monkeypatch.setattr(confluence_import, "create_pool", AsyncMock(side_effect=RuntimeError("redis")))
    ...
    session.get = AsyncMock(return_value=SimpleNamespace(status="queued", archive_id=archive.id, import_all=False, space_keys=[], counters={}))
    with pytest.raises(ConflictError):
        await confluence_import.retry(uuid.uuid4(), actor, session)
