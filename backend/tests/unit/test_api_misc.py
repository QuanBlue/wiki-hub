...
def _user() -> SimpleNamespace:
    ...
    return SimpleNamespace(
        id=uuid.uuid4(), username="admin", email="admin@example.com", full_name="Admin",
...
async def test_auth_route_wrappers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(auth.SiteSettingsService, "get_effective", AsyncMock(return_value=effective))
    monkeypatch.setattr(auth, "create_access_token", lambda *args, **kwargs: ("token", datetime.now(UTC)))
    request = Request({"type": "http", "client": ("127.0.0.1", 1234), "headers": [], "method": "POST", "path": "/"})
    response = Response()
    result = await auth.login(SimpleNamespace(username="Admin", password="password"), request, response, service, Mock())
    assert result.access_token == "token"
    await auth.logout(response)
    await auth.renew(response, user, None, Mock())
    me = await auth.me(user, None, Mock())
    assert me.id == user.id
    impersonated = await auth.start_impersonation(SimpleNamespace(user_id=user.id), response, service, Mock())
    assert impersonated.user.id == user.id
    ended = await auth.stop_impersonation(response, service, Mock())
    assert ended.user.id == user.id


@pytest.mark.asyncio
async def test_storage_attachment_backup_and_settings_wrappers(monkeypatch: pytest.MonkeyPatch) -> None:
    user = _user()
    storage = Mock()
    ...
    assert (await storage_admin.presign_download(user, storage, key="a%20b.txt", inline=False)).url == "https://download"
    ...
    monkeypatch.setattr(attachments, "SpaceService", lambda _session: SimpleNamespace(permissions=SimpleNamespace(require=AsyncMock())))
    monkeypatch.setattr(attachments, "PageService", lambda _session: SimpleNamespace(require_page_view=AsyncMock()))
    response = await attachments.read_attachment(uuid.uuid4(), user, session, storage)
    assert response.media_type == "text/plain" and "attachment" in response.headers["content-disposition"]
...
async def test_group_user_audit_and_meta_wrappers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert await groups.list_groups(user, session, service, q=None) == [SimpleNamespace(id=group.id)]
    ...
    monkeypatch.setattr(meta.SiteSettingsService, "env_defaults", classmethod(lambda cls: SimpleNamespace(site_name="WikiHub", max_upload_size_bytes=1, allowed_attachment_types=[])))
    ...
    assert (await users.change_own_password(SimpleNamespace(current_password="x", new_password="password"), user, user_service)).id == user.id
