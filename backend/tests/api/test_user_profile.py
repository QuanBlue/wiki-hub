...
async def test_member_profile_includes_contact_and_activity_fields(
    ...
    response = await client.get(f"/api/v1/users/{api_user.username}/profile")

    assert response.status_code == 200
    profile = response.json()
    assert profile["username"] == api_user.username
    assert profile["full_name"] == api_user.full_name
