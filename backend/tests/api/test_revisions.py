...
@pytest.mark.asyncio
async def test_page_revisions_workflow(
    ...
    rev_list_resp = await client.get(f"/api/v1/spaces/{space_key}/pages/{slug}/revisions")
    ...
    update_resp = await client.patch(
        ...
        json={
            "title": "Page Version Two",
            "content": "<p>Updated content line 1</p>\n<p>Updated content line 2</p>",
        },
    )
    assert update_resp.status_code == 200

    # 5. List revisions again (should have 2 items: v2 and v1)
    rev_list_resp2 = await client.get(f"/api/v1/spaces/{space_key}/pages/{slug}/revisions")
    ...
    rev_list_resp3 = await client.get(f"/api/v1/spaces/{space_key}/pages/{slug}/revisions")
