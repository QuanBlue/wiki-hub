"""Fix and normalize Confluence attachment display modes and view-file macros on existing pages.
...
down_revision: str = "20260821_0004"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    results = connection.execute(
        sa.select(pages_table.c.id, pages_table.c.content).where(
            sa.or_(
                pages_table.c.content.like("%confluence-macro-view-file%"),
                pages_table.c.content.like("%view-file%"),
                pages_table.c.content.like("%viewfile%"),
                pages_table.c.content.like("%view-doc%"),
                pages_table.c.content.like("%viewdoc%"),
            )
        )
    ).fetchall()
